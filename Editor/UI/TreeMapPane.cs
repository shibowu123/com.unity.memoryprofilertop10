...
namespace Unity.MemoryProfiler.Editor.UI
{
    internal class TreeMapPane : ViewPane, IDisposable
    {
        ...
        [MenuItem("MemoryProfiler2/Dump Top 10 Statistics")]
        public static void DumpTop10Statistics()
        {
            var uiState = MemoryProfilerWindow.GetWindow<MemoryProfilerWindow>().UIState;
            if (uiState == null || uiState.snapshotMode == null || uiState.snapshotMode.snapshot == null)
            {
                Debug.Log("请打开内存快照.");
                return;
            }
            ...
            var now = new DateTimeOffset(DateTime.Now).ToUnixTimeSeconds();
            var filename = $"{now}_Top10_Statistics.csv";
            var toFile = EditorUtility.SaveFilePanel("Save", "", filename, "");
            if (toFile == "") return;

            var f = new StreamWriter(toFile);

            // 1. Unity Objects Top 10 By Memory
            var unityTypeStats = new Dictionary<string, (ulong totalSize, int count)>();
            for (int i = 0; i < snapshot.NativeObjects.Count; ++i)
            {
                var size = snapshot.NativeObjects.Size[i];
                var typeIndex = snapshot.NativeObjects.NativeTypeArrayIndex[i];
                if (typeIndex >= 0)
                {
                    var typeName = snapshot.NativeTypes.TypeName[typeIndex];
                    if (unityTypeStats.ContainsKey(typeName))
                    {
                        var stats = unityTypeStats[typeName];
                        unityTypeStats[typeName] = (stats.totalSize + size, stats.count + 1);
                    }
                    else
                    {
                        unityTypeStats[typeName] = (size, 1);
                    }
                }
            }

            var unityTop10ByMemory = unityTypeStats.OrderByDescending(x => x.Value.totalSize).Take(10).ToList();
            f.WriteLine("Unity Objects Top 10 By Memory");
            f.WriteLine("TypeName,TotalSize(MB)");
            foreach (var item in unityTop10ByMemory)
            {
                var sizeInMB = item.Value.totalSize / (1024.0 * 1024.0);
                f.WriteLine($"{item.Key},{sizeInMB:F3}");
            }
            f.WriteLine();

            // 2. Unity Objects Top 10 By Count
            var unityTop10ByCount = unityTypeStats.OrderByDescending(x => x.Value.count).Take(10).ToList();
            f.WriteLine("Unity Objects Top 10 By Count");
            f.WriteLine("TypeName,Count");
            foreach (var item in unityTop10ByCount)
            {
                f.WriteLine($"{item.Key},{item.Value.count}");
            }
            f.WriteLine();

            // 3. Managed Objects Top 10 By Memory
            var managedTypeStats = new Dictionary<string, (ulong totalSize, int count)>();
            ...
            for (int i = 0; i < managedObjects.Count; ++i)
            {
                ...
                if (managedObject.Size > 0)
                {
                    ...
                    if (snapshot.TypeDescriptions.PureCSharpTypeIndices.Contains(typeIndex))
                    {
                        var typeName = snapshot.TypeDescriptions.TypeDescriptionName[typeIndex];
                        if (managedTypeStats.ContainsKey(typeName))
                        {
                            var stats = managedTypeStats[typeName];
                            managedTypeStats[typeName] = (stats.totalSize + (ulong)managedObject.Size, stats.count + 1);
                        }
                        else
                        {
                            managedTypeStats[typeName] = ((ulong)managedObject.Size, 1);
                        }
                    }
                }
            }

            var managedTop10ByMemory = managedTypeStats.OrderByDescending(x => x.Value.totalSize).Take(10).ToList();
            f.WriteLine("Managed Objects Top 10 By Memory");
            f.WriteLine("TypeName,TotalSize(MB)");
            foreach (var item in managedTop10ByMemory)
            {
                var sizeInMB = item.Value.totalSize / (1024.0 * 1024.0);
                f.WriteLine($"{item.Key},{sizeInMB:F3}");
            }
            ...
            // 4. Managed Objects Top 10 By Count
            var managedTop10ByCount = managedTypeStats.OrderByDescending(x => x.Value.count).Take(10).ToList();
            f.WriteLine("Managed Objects Top 10 By Count");
            f.WriteLine("TypeName,Count");
            foreach (var item in managedTop10ByCount)
            {
                f.WriteLine($"{item.Key},{item.Value.count}");
            }

            f.Close();
            Debug.Log($"Dump Top 10 Statistics: {toFile}");
        }
